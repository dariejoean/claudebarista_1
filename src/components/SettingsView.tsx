...
import { TrashIcon, ArrowDownTrayIcon, ArrowUpTrayIcon, TableCellsIcon, SwatchIcon, PaintBrushIcon, HandThumbUpIcon, HandThumbDownIcon, SparklesIcon, DevicePhoneMobileIcon, ExclamationTriangleIcon, CheckBadgeIcon, CloudArrowUpIcon, WrenchScrewdriverIcon, BoltIcon, FunnelIcon } from '@heroicons/react/24/solid';
import { EspressoMachineIcon, TamperIcon, CoffeeBeansIcon, WaterDropIcon, GrinderIcon, AccessoriesIcon } from '../components/CustomIcons';
...
export const SettingsView: React.FC<SettingsViewProps> = React.memo((props) => {
  ...
  const { isExporting, handleExportExcel, handleExportCSV, handleBackupLocal, handleBackupCloud, handleRestore, handleClearAllData } = useDataManagement(props.shots);
  ...
  return (
    <div className="animate-fade-in pb-20 space-y-6">
      ...
      <div className="grid grid-cols-2 gap-4">
        ...
        <button onClick={handleExportCSV} disabled={isExporting} className={BIG_BUTTON_STYLE}>
          {isExporting ? <div className="animate-spin w-8 h-8 border-2 border-teal-400 border-t-transparent rounded-full" /> : <TableCellsIcon className={getIconStyle('text-teal-400')} />}
          <span className={BUTTON_TEXT_STYLE}>{isExporting ? 'SE EXPORTĂ...' : 'EXPORT CSV'}</span>
        </button>
      </div>
      <label className={`${BIG_BUTTON_STYLE} cursor-pointer w-full`}>
        <ArrowUpTrayIcon className={getIconStyle('text-violet-400')} />
      ...
      <button onClick={ctrl.toggleHaptics} className={`w-full py-4 bg-surface-container rounded-full flex items-center justify-between px-6 shadow-md transition-all border mb-3 ${ctrl.hapticsOn ? 'border-indigo-500 bg-indigo-500/10' : 'border-white/5'}`}>
